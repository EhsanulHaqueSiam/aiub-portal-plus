...
import { escHtml, loadCSS, loadInlineStyle, replaceChildrenHTML } from '@/utils/portal';

declare global {
  interface Window {
    __aiubRegEnhanced?: boolean;
  }
}

type CreditItem = { key: string; val: string };
type Schedule = { type: string; time: string; room: string };
type Course = {
  code: string;
  name: string;
  section: string;
...
type FeeType = 'normal' | 'total' | 'net-total' | 'deduction' | 'prev' | 'balance' | 'paid';
type FeeItem = { label: string; amt: string; type: FeeType };
type SemOpt = { val: string; text: string; selected: boolean };
...
.reg-wrap { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Inter', Roboto, sans-serif; font-size: 13px; color: #1e293b; }
.reg-top { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; margin-bottom: 16px; padding-bottom: 14px; border-bottom: 2px solid #f1f5f9; }
...
.reg-sem-select { font-size: 12px; font-weight: 600; color: #334155; border: 1px solid #cbd5e1; border-radius: 8px; padding: 6px 10px; background: #f8fafc; cursor: pointer; outline: none; transition: border-color .15s, box-shadow .15s; }
.reg-sem-select:focus { border-color: #93c5fd; box-shadow: 0 0 0 3px rgba(59,130,246,.1); }
.reg-print-btn { font-size: 11px; font-weight: 600; color: #475569; text-decoration: none; border: 1px solid #cbd5e1; border-radius: 6px; padding: 5px 13px; background: #f8fafc; transition: background .15s, color .15s, border-color .15s; white-space: nowrap; }
...
.reg-credit-chip { display: inline-flex; align-items: center; gap: 5px; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 600; background: #f1f5f9; border: 1px solid #e2e8f0; color: #94a3b8; }
...
.reg-course-card { border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden; transition: box-shadow .15s, border-color .15s; background: #fff; }
...
.reg-course-code { font-family: 'Consolas', 'Cascadia Code', 'Courier New', monospace; font-size: 11px; font-weight: 600; color: #1e3a8a; margin-bottom: 3px; letter-spacing: .3px; }
...
.reg-section-badge { flex-shrink: 0; width: 30px; height: 30px; border-radius: 8px; background: #eff6ff; border: 1px solid #bfdbfe; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 800; color: #1d4ed8; }
...
.reg-sched-type { display: inline-block; padding: 1px 7px; border-radius: 4px; font-size: 10px; font-weight: 700; white-space: nowrap; flex-shrink: 0; }
...
.reg-sched-room { margin-left: auto; font-size: 10px; font-weight: 700; color: #94a3b8; white-space: nowrap; font-family: 'Consolas', monospace; }

.reg-course-footer { display: flex; align-items: center; justify-content: space-between; padding: 5px 14px 9px; gap: 8px; }
.reg-credits-tag { font-size: 10px; color: #94a3b8; font-family: 'Consolas', monospace; font-weight: 600; }
.reg-drop-badge { display: inline-flex; align-items: center; gap: 4px; background: #fee2e2; color: #dc2626; border-radius: 5px; padding: 2px 8px; font-size: 10px; font-weight: 700; }

.reg-fee-panel { border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden; background: #fff; position: sticky; top: 16px; }
.reg-fee-head { padding: 10px 14px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .7px; color: #475569; }
.reg-fee-list { padding: 4px 0; }
.reg-fee-row { display: flex; align-items: center; justify-content: space-between; padding: 5px 14px; gap: 8px; }
...
.reg-fee-row.deduction .reg-fee-lbl, .reg-fee-row.deduction .reg-fee-amt { color: #d97706; }
.reg-fee-row.prev .reg-fee-lbl, .reg-fee-row.prev .reg-fee-amt { color: #d97706; }
...
function parseCreditSummary(tbl: Element): CreditItem[] {
  const items: CreditItem[] = [];
  tbl.querySelectorAll('div[class*="col-"]').forEach((col) => {
    const labels = Array.from(col.children).filter(
      (el) => el.tagName === 'LABEL',
    ) as HTMLLabelElement[];
    if (labels.length >= 2) {
...
function parseScheduleLine(text: string): Schedule {
  const typeMatch = text.match(/^\(([^)]+)\)/);
  const type = typeMatch ? typeMatch[1] : '';
  const afterType = text.replace(/^\([^)]+\)\s*/, '');
  const roomIdx = afterType.lastIndexOf('Room:');
  const time = (roomIdx > 0 ? afterType.slice(0, roomIdx) : afterType)
    .replace(/^Time:\s*/, '')
    .trim();
  const room = roomIdx > 0 ? afterType.slice(roomIdx + 5).trim() : '';
  return { type, time, room };
}

function parseCourses(table: Element): Course[] {
  ...
  table.querySelectorAll('tbody tr').forEach((tr) => {
    ...
    firstTd.querySelectorAll('div').forEach((d) => {
      const span = d.querySelector('span') as HTMLSpanElement | null;
      if (!span || span.style.color === 'red') return;
      const txt = span.textContent?.trim();
      if (txt) schedules.push(parseScheduleLine(txt));
    });

    const dropSpan = firstTd.querySelector('span[style*="color: red"]');
    const droppedText = dropSpan?.textContent?.trim() ?? '';
    const credStr = tds[1]?.textContent?.trim() ?? '';
...
function parseFees(div: Element): FeeItem[] {
  const items: FeeItem[] = [];
...
function schedClass(type: string): string {
...
function courseCardHTML(c: Course): string {
  ...
  const schedHTML = c.schedules
    .map(
      (s) => `<div class="reg-schedule-row">
        <span class="reg-sched-type ${schedClass(s.type)}">${escHtml(s.type || 'Time')}</span>
        <span class="reg-sched-time">${escHtml(s.time)}</span>
        ${s.room ? `<span class="reg-sched-room">${escHtml(s.room)}</span>` : ''}
      </div>`,
    )
    .join('');
      ...
      <div class="reg-course-head">
        <div class="reg-course-info">
          <div class="reg-course-code">
            <a href="${escHtml(safeHref)}" style="color:inherit;text-decoration:none">${escHtml(c.code)}</a>
          </div>
          <div class="reg-course-name">${escHtml(c.name)}</div>
        </div>
        ${c.section ? `<div class="reg-section-badge">${escHtml(c.section)}</div>` : ''}
      </div>
      ${schedHTML ? `<div class="reg-schedule">${schedHTML}</div>` : ''}
      <div class="reg-course-footer">
        <span class="reg-credits-tag">${escHtml(c.credStr)}</span>
        ${isDropped ? `<span class="reg-drop-badge">&#9888; Dropped ${escHtml(dropDetail)}</span>` : ''}
      </div>
    </div>`;
}

function feePanelHTML(items: FeeItem[]): string {
  if (!items.length) return '';
  const rowsHTML = items
    .map((item) => {
      const num = parseFloat(item.amt.replace(/,/g, ''));
      const isZero = !isNaN(num) && num === 0 && item.type === 'normal';
      let cls: string = item.type;
      if (item.type === 'balance') cls += num > 0 ? ' owe' : ' clear';

      const needsDivider =
        item.type === 'total' || item.type === 'net-total' || item.type === 'balance';
      const divider = needsDivider ? '<div class="reg-fee-divider"></div>' : '';
      return `${divider}
        <div class="reg-fee-row ${cls}${isZero ? ' zero' : ''}">
          <span class="reg-fee-lbl">${escHtml(item.label)}</span>
          <span class="reg-fee-amt">${escHtml(item.amt)}</span>
        </div>`;
    })
    .join('');
  return `
...
export default defineContentScript({
  matches: ['https://portal.aiub.edu/Student/Registration*'],
  runAt: 'document_idle',
  ...
  async main() {
    ...
    const init = () => {
      ...
      loadCSS('aiub-reg-external', 'Academic/Registration.css');
      loadInlineStyle('aiub-reg-css', AIUB_REG_CSS);
      enhance();
    };
...
function enhance() {
  const panel =
    document.querySelector<HTMLElement>('#main-content .margin5 .panel.panel-default') ??
    document.querySelector<HTMLElement>('#main-content .panel.panel-default');
  if (!panel) return;

  panel.classList.add('reg-root-panel');

  const origSelect = panel.querySelector<HTMLSelectElement>('#SemesterDropDown');
  const semOptions: SemOpt[] = origSelect
    ? Array.from(origSelect.querySelectorAll('option')).map((o) => ({
        val: o.value,
  ...
  const printBtn = panel.querySelector<HTMLAnchorElement>('a.btn-danger');
  const printHref = printBtn?.getAttribute('href') ?? null;
  ...
  const semOptsHTML = semOptions
    .map(
      (o) =>
        `<option value="${escHtml(o.val)}"${o.selected ? ' selected' : ''}>${escHtml(o.text)}</option>`,
    )
    .join('');

  const creditChips = creditItems
    .map(({ key, val }) => {
      const active = parseFloat(val) > 0;
      return `<span class="reg-credit-chip${active ? ' active' : ''}">
        <span class="reg-credit-num">${escHtml(val)}</span>${escHtml(key)}
      </span>`;
    })
    .join('');
    ...
    <div class="reg-wrap">
      <div class="reg-top">
        ...
        <div class="reg-top-right">
          ...
          ${safeHref ? `<a class="reg-print-btn" href="${escHtml(safeHref)}">&#128438; Print</a>` : ''}
        </div>
      </div>
  ...
  if (panelBody) replaceChildrenHTML(panelBody, html);

  const newSelect = panel.querySelector<HTMLSelectElement>('#reg-sem-select');
  newSelect?.addEventListener('change', () => {
    if (newSelect.value) window.location.href = newSelect.value;
  });
}
