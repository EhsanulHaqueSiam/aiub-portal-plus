...
declare global {
  interface Window {
    __aiubSemGradeEnhanced?: boolean;
  }
}
...
type CourseState = 'done' | 'ong' | 'wdn' | 'fail';
type Course = {
  classId: string;
  name: string;
  credits: string;
...
type Summary = { tgp: string; ecr: string; gpa: string; cgpa: string };
type Semester = { label: string; courses: Course[]; summary: Summary | null };

function gpaColor(v: string): string {
...
const normText = (v: unknown) => String(v ?? '').replace(/\s+/g, ' ').trim().toUpperCase();
const normCode = (v: unknown) => normText(v).replace(/\s+/g, '');
...
function persistSemesterGraphData(infoItems: InfoItem[], semesters: Semester[]) {
  saveGraphData((store) => {
    const curriculum = (store.curriculum ?? {}) as {
    ...
    const isCoreCourse = (course: Course) => {
      if (!hasCoreMap) return true;
      return coreCodeSet.has(normCode(course.classId)) || coreNameSet.has(normText(course.name));
    };
    ...
    const sumCredit = (courses: Course[], pred: (c: Course) => boolean) =>
      courses.filter(pred).reduce((sum, c) => sum + parseCreditValue(c.creditValue), 0);
    ...
    const semesterGpaTrend = filteredSemesters
      .map((sem) => ({
        label: sem.label,
        gpa: extractNumber(sem.summary?.gpa),
      }))
      .filter((p) => p.gpa > 0);
    ...
    const cgpaTrend = filteredSemesters
      .map((sem) => ({
        label: sem.label,
        cgpa: extractNumber(sem.summary?.cgpa),
      }))
      .filter((p) => p.cgpa > 0);
    ...
    // Per-course completion snapshot — name-only since BySemester has no
    // course codes, just class IDs. The Routine Generator uses this to
    // compute eligibility against curriculum data (which HAS codes).
    const courseStates = semesters.flatMap((sem) =>
      sem.courses.map((c) => ({
        label: sem.label,
        classId: c.classId,
        name: c.name,
        nameNorm: normText(c.name),
        credit: c.credits,
        grade: c.fg,
        state: c.state,
      })),
    ...
    // Consolidate by normalized name: if a course appears multiple times
    // (retakes), the best state wins (done > ong > fail > wdn).
    const priority: Record<string, number> = { done: 4, ong: 3, fail: 2, wdn: 1 };
    const bestByName = new Map<string, { name: string; state: string; grade: string }>();
    for (const s of courseStates) {
      if (!s.nameNorm) continue;
      const prev = bestByName.get(s.nameNorm);
      const p = priority[s.state] ?? 0;
      if (!prev || p > (priority[prev.state] ?? 0)) {
        bestByName.set(s.nameNorm, { name: s.name, state: s.state, grade: s.grade });
      }
    }
    const completedNames = Array.from(bestByName.values())
      .filter((x) => x.state === 'done' || x.state === 'ong')
      .map((x) => normText(x.name));
...
function parseInfo(tbl: Element): InfoItem[] {
  ...
  tbl.querySelectorAll('tr').forEach((tr) => {
    ...
    if (tds[0] && tds[2])
      items.push({ k: tds[0].textContent?.trim() ?? '', v: tds[2].textContent?.trim() ?? '' });
    if (tds[3] && tds[5])
      items.push({ k: tds[3].textContent?.trim() ?? '', v: tds[5].textContent?.trim() ?? '' });
  });
  return items;
}

function parseSemesters(tbl: Element): Semester[] {
  const sems: Semester[] = [];
  let cur: Semester | null = null;
...
function gradePill(fg: string): string {
  if (!fg) return '<span class="sgr-gp-nd">—</span>';
  if (fg === '-') return '<span class="sgr-gp" style="color:#7c3aed">Ongoing</span>';
  const bg = GRADE_BG[fg] ?? '#6b7280';
  return `<span class="sgr-gp" style="color:${bg}">${escHtml(fg)}</span>`;
}

function miniGrade(g: string): string {
  if (!g || g === '-') return '<span class="sgr-mini-nd">—</span>';
  const bg = GRADE_BG[g] ?? '#6b7280';
  return `<span class="sgr-mini" style="color:${bg}">${escHtml(g)}</span>`;
}

function statusBadge(state: CourseState): string {
  const map: Record<CourseState, string> = {
...
function infoHTML(items: InfoItem[], printHref: string | null): string {
    ...
    .map(
      ({ k, v }) =>
        `<div class="sgr-info-cell">
          <div class="sgr-info-lbl">${escHtml(k)}</div>
          <div class="sgr-info-val${k === 'Cgpa' ? ' cgpa' : ''}">${escHtml(v) || '—'}</div>
        </div>`,
    )
    .join('');
  ...
  if (printHref) {
    actions.push(`<a class="sgr-action-link sgr-print" href="${escHtml(safeHref)}">&#128438; Print</a>`);
  }
  if (graphHref) {
    actions.push(`<a class="sgr-action-link sgr-graph" href="${escHtml(graphHref)}">&#128202; Graph</a>`);
  }
...
function semesterHTML(sem: Semester): string {
    ...
    ? `
      <div class="sgr-sum-bar">
        <div class="sgr-sum-item">
          ...
          <span class="sgr-sum-val">${escHtml(sem.summary.tgp || '—')}</span>
        </div>
        <div class="sgr-sum-item">
          <div class="sgr-sum-lbl">Credits Earned</div>
          <span class="sgr-sum-val">${escHtml(sem.summary.ecr || '—')}</span>
        </div>
        <div class="sgr-sum-item">
          <div class="sgr-sum-lbl">Semester GPA</div>
          <span class="sgr-sum-val" style="color:${gpaColor(sem.summary.gpa)}">${escHtml(sem.summary.gpa || '—')}</span>
        </div>
        <div class="sgr-sum-item">
          <div class="sgr-sum-lbl">Cumulative GPA</div>
          <span class="sgr-sum-val" style="color:${gpaColor(sem.summary.cgpa)}">${escHtml(sem.summary.cgpa || '—')}</span>
        </div>
      </div>`
    : '';
    ...
    <div class="sgr-sem-card${isActive ? ' sgr-sem-card--active' : ''}">
      <div class="sgr-sem-head">
        <div class="sgr-sem-label">
          <span class="sgr-sem-dot"></span>${escHtml(sem.label)}
        </div>
        <div class="sgr-sem-meta">
          ...
          ${
            sem.summary && sem.summary.gpa && sem.summary.gpa !== '0.00'
              ? `<span class="sgr-sem-gpa">GPA&thinsp;<strong>${escHtml(sem.summary.gpa)}</strong></span>`
              : ''
          }
...
function enhance() {
  ...
  gr.closest('.panel')?.classList.add('sgr-root-panel');
  ...
  const tables = Array.from(gr.querySelectorAll<HTMLTableElement>('table'));
  ...
  gr.addEventListener('click', (e) => {
    ...
    const head = target.closest('.sgr-sem-head');
    head?.closest('.sgr-sem-card')?.classList.toggle('collapsed');
  });
}
...
export default defineContentScript({
  ...
  async main() {
    ...
    loadCSS('sgr-style', 'Grade/carriculum_grade_semester.css');
