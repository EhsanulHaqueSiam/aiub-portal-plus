...
import { loadCSS, loadInlineStyle, beforeHTML } from '@/utils/portal';

declare global {
  interface Window {
    __aiubFinanceEnhanced?: boolean;
  }
}
...
.fin-card { flex: 1; min-width: 150px; background: #fff; border-radius: 12px; padding: 14px 18px; border: 1.5px solid #e5e7eb; box-shadow: 0 1px 4px rgba(0,0,0,.05); }
...
.fin-table thead th { padding: 12px 14px; font-weight: 600; font-size: 11px; color: #fff; text-transform: uppercase; letter-spacing: 0.5px; border: none !important; white-space: nowrap; }
...
.fin-badge { display: inline-block; font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 999px; margin-right: 7px; vertical-align: middle; text-transform: uppercase; letter-spacing: 0.3px; white-space: nowrap; }
...
.fin-modal .modal-dialog { max-width: 620px; margin: 40px auto; border-radius: 14px; overflow: hidden; }
.fin-modal .modal-content { border: none; border-radius: 14px; box-shadow: 0 8px 32px rgba(0,0,0,.18); }
.fin-modal .modal-header { background: linear-gradient(135deg, #1e3a5f 0%, #1d4ed8 100%); border: none; padding: 18px 24px; border-radius: 14px 14px 0 0; }
...
.fin-modal-table thead th { padding: 10px 12px; font-weight: 600; font-size: 12px; color: #374151; background: #f1f5f9; border-bottom: 2px solid #e2e8f0; text-transform: uppercase; letter-spacing: 0.4px; }
...
.fin-modal .modal-body h3, .fin-modal .modal-body h4, .fin-modal .modal-body h5 { color: #1d4ed8; font-weight: 700; margin: 0 0 12px; font-size: 15px; }
`;
...
function fmtAmt(num: number): string {
...
function enhance() {
  ...
  const table = panelBody.querySelector<HTMLTableElement>('table.table-details');
  if (!table) return;

  panelBody.closest('.panel')?.classList.add('fin-root-panel');
  ...
  table.querySelectorAll<HTMLTableRowElement>('tbody tr').forEach((row) => {
    ...
    if (cells[0].hasAttribute('colspan')) {
      ...
      if (cells.length >= 5) {
        totalDebit = parseAmount(
          cells[1].querySelector('label')?.textContent ?? cells[1].textContent,
        );
        ...
        cells[4].classList.add(finalBalance === 0 ? 'fin-zero' : 'fin-positive');
      }
      return;
    }
    ...
    if (modalLink) {
      ...
      const badge = document.createElement('span');
      badge.className = 'fin-badge fin-badge-payment';
      badge.textContent = 'Payment';
      particularsCell.insertBefore(badge, particularsCell.firstChild);
    } else {
      row.classList.add('fin-misc-row');
      const badge = document.createElement('span');
      badge.className = 'fin-badge fin-badge-misc';
...
export default defineContentScript({
  matches: ['https://portal.aiub.edu/Student/Accounts*'],
  runAt: 'document_idle',
  ...
  async main() {
    ...
    const tryEnhance = () => {
      if (document.querySelector('#main-content .panel-body table.table-details')) {
        loadCSS('fin-external', 'Academic/Financials.css');
        loadInlineStyle('fin-style', FIN_CSS);
        enhance();
      } else {
