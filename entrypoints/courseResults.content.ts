...
import { escHtml, loadInlineStyle, replaceChildrenHTML } from '@/utils/portal';

declare global {
  interface Window {
    __aiubCourseEnhanced?: boolean;
  }
}
...
const gradeColor = (g: string) => GRADE_COLORS[(g || '-').trim()] || '#6b7280';

type Meta = { total: string; pass: string; contrib: string };
type GradeScore = { grade: string; score: string };
type SubItem = { name: string; meta: Meta; score: string; isH4: boolean };
type Section = SubItem & { subItems: SubItem[] };
type Term = {
  termName: string;
  termMeta: Meta;
  grade: string;
...
type SelectOpt = { value: string; text: string; selected: boolean };
...
function parseGradeScore(text: string): GradeScore {
  const m = (text || '').trim().match(/^(\S+)\s*\(([^)]*)\)/);
  return m ? { grade: m[1], score: m[2] } : { grade: (text || '-').trim(), score: '' };
}

function metaLine(m: Meta): string {
  if (!m.total) return '';
  return `Total: ${escHtml(m.total)} &nbsp;·&nbsp; Pass: ${escHtml(m.pass)} &nbsp;·&nbsp; Contributes: ${escHtml(m.contrib)}`;
}

function selectOptions(sel: HTMLSelectElement): SelectOpt[] {
...
function parseRow(row: Element): SubItem | null {
  const hEl = row.querySelector('h4, h6');
  if (!hEl) return null;
  const clone = hEl.cloneNode(true) as Element;
...
function parseTerm(termEl: Element): Term | null {
  ...
  const gs = parseGradeScore(
    bgRow.querySelector('.col-md-4 h4, .col-sm-4 h4')?.textContent?.trim() ?? '-',
  );
...
function buildSelectOpts(opts: SelectOpt[]): string {
  return opts
    .map(
      (o) =>
        `<option value="${escHtml(o.value)}"${o.selected ? ' selected' : ''}>${escHtml(o.text)}</option>`,
    )
    .join('');
}

function termCardHTML(term: Term, idx: number): string {
  let sectionsHTML = '';
...
<div class="car-term-card ${termClass} car-open" id="car-term-${idx}">
  <div class="car-term-head">
    ...
    <div class="car-term-right">
      ${term.score && term.score !== '-' ? `<span class="car-term-score">${escHtml(term.score)}</span>` : ''}
      <span class="car-term-grade" style="color:${gradeColor(term.grade)}">${escHtml(term.grade)}</span>
...
.car-term-card.car-midterm:hover { border-color: #93c5fd; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.15); }
...
.car-term-card.car-finalterm:hover { border-color: #f472b6; box-shadow: 0 4px 12px rgba(236, 72, 153, 0.15); }

.car-term-head { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; padding: 16px 18px; cursor: pointer; user-select: none; background: transparent; border-bottom: 1px solid transparent; transition: background 0.12s; }
.car-term-card.car-midterm .car-term-head { border-bottom: 1px solid #dbeafe; }
.car-term-card.car-finalterm .car-term-head { border-bottom: 1px solid #fbcfe8; }
.car-term-head:hover { opacity: 0.85; }
.car-term-label { display: inline-block; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; padding: 3px 10px; border-radius: 5px; white-space: nowrap; margin-right: 6px; }
.car-term-card.car-midterm .car-term-label { background: #dbeafe; color: #1d4ed8; }
.car-term-card.car-finalterm .car-term-label { background: #fbcfe8; color: #be185d; }
...
.car-term-chevron { font-size: 11px; transition: transform 0.2s; padding: 6px 8px; border-radius: 5px; }
...
.car-section-card { margin: 0; padding: 0; border: none; border-radius: 6px; overflow: hidden; background: rgba(255, 255, 255, 0.4); transition: all 0.12s; }
.car-section-card:not(:last-child) { margin-bottom: 10px; }
.car-section-head { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; background: transparent; cursor: pointer; user-select: none; border: none; transition: background 0.12s; }
.car-section-card:hover { background: rgba(255, 255, 255, 0.65); }
.car-term-card.car-midterm .car-section-card:hover { background: rgba(59, 130, 246, 0.08); }
.car-term-card.car-finalterm .car-section-card:hover { background: rgba(236, 72, 153, 0.08); }
...
.car-section-chevron { font-size: 11px; transition: transform 0.2s; padding: 4px 6px; border-radius: 4px; }
...
.car-section-card.car-open .car-section-body { display: block; padding: 8px 14px; background: rgba(255, 255, 255, 0.3); }

.car-sub-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; border: none; border-radius: 4px; background: rgba(255, 255, 255, 0.2); transition: background 0.1s; }
...
export default defineContentScript({
  matches: ['https://portal.aiub.edu/Student/Course*'],
  runAt: 'document_idle',
...
function enhance() {
  const panelBody = document.querySelector<HTMLElement>('#main-content .panel-body');
  if (!panelBody) return;

  const sectionDdEl = document.getElementById('SectionDropDown') as HTMLSelectElement | null;
  const semesterDdEl = document.getElementById('SemesterDropDown') as HTMLSelectElement | null;
  if (!sectionDdEl || !semesterDdEl) return;

  const listGroup = panelBody.querySelector('.list-group');
  if (!listGroup) return;

  const activeItem = listGroup.querySelector<HTMLElement>('.list-group-item.active');
  if (!activeItem) return;

  loadInlineStyle('car-style', CAR_CSS);
  ...
  const finalGradeRaw =
    activeItem.querySelector('.col-md-4 h4, .col-sm-4 h4')?.textContent?.trim() ?? '-';
  ...
  const terms: Term[] = Array.from(termEls)
    .map(parseTerm)
...
<div class="car-course-card">
  ...
  replaceChildrenHTML(panelBody, filterBar + courseCard + termsHTML);

  panelBody.closest('.panel')?.classList.add('car-root-panel');

  const sectionSel = document.getElementById('car-section-dd') as HTMLSelectElement | null;
  const semesterSel = document.getElementById('car-semester-dd') as HTMLSelectElement | null;
  sectionSel?.addEventListener('change', () => {
    if (sectionSel.value) window.location.href = sectionSel.value;
  });
  semesterSel?.addEventListener('change', () => {
    if (semesterSel.value) window.location.href = semesterSel.value;
  });

  panelBody.addEventListener('click', (e) => {
    const target = e.target as HTMLElement;
    const termHead = target.closest('.car-term-head');
    if (termHead) {
      termHead.closest('.car-term-card')?.classList.toggle('car-open');
      return;
    }
    const secHead = target.closest('.car-section-head');
    if (secHead) {
